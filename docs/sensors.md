...
* 먼저 센서의 `name`과 `type`을 정의한 다음 `lidar_frame`을 기준으로 `<pose>`를 정의했습니다.
* `<topic>`에서는 lidar 데이터가 게시될 토픽을 정의합니다.
* `<update_rate>`는 `lidar` 데이터가 생성되는 빈도로, 이 경우 `10Hz`이며 `0.1초`와 같습니다.
* `<horizontal>` 및 `<vertical>` 태그 아래에서 수평 및 수직 레이저 광선의 속성을 정의합니다.
* `<samples>`는 전체 레이저 스윕 주기당 생성할 시뮬레이션된 lidar 광선의 수입니다.
* `<resolution>`: 이 숫자는 샘플과 곱하여 범위 데이터 포인트 수를 결정합니다.
* `<min_angle>` 및 `<max_angle>`은 생성된 광선의 각도 범위입니다.
* `<range>` 아래에서 각 시뮬레이션된 광선의 범위 속성을 정의합니다.
    * `<min>` 및 `<max>`는 각 lidar 광선의 최소 및 최대 거리를 정의합니다.
    * 여기서 `<resolution>` 태그는 각 lidar 광선의 선형 해상도를 정의합니다.
 * `<always_on>`: 참이면 센서가 `<update_rate>`에 따라 항상 업데이트됩니다.
 * `<visualize>`: 참이면 센서가 GUI에 시각화됩니다.
